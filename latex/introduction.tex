...
However, despite their great suitability and acceptance, triangle-based representations also have their drawbacks and a numerous amount of problems can be addressed better using different, often implicit, methods or representations (\cf examples by Menon \cite{implicit_techniques} and Tukora \cite{virtual_machining_review}). %TODO write some examples down
...
The focus of the implementations underlying this thesis is to find, evaluate and prototypically implement multiple strategies to extract a surface mesh from the data model used inside the VML.
...
The goal of this thesis is to provide the reader with a state of the art overview over algorithms and methods used to reconstruct explicit triangle meshes from different kinds of implicit geometry models.
