...
\begin{description}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[t]{0.32\textwidth}
			...
			\caption{Vertex with a triangle fan and one additional triangle, non-manifold.}
			\label{fig:non_manifold_vertex}
		\end{subfigure}
		\begin{subfigure}[t]{0.32\textwidth}
			\centering
			\includegraphics[width=\textwidth]{images/t_vertex}
			\caption{Vertex $E$ lies directly on edge $EC$. There is either a degenerate triangle $AEC$ (zero area) or triangle $ABC$ should be a quadrilateral with $E$ and should be split along $EB$.}
			\label{fig:t_vertex}
		\end{subfigure}
		\caption{
	...
	\begin{figure}[H]
		...
		\caption{
			...
			Image by Kreveld and Loffler \cite{delaunay_image}.
		}
