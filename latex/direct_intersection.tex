...
Due to the triangle elimination using cell classification, \cf \cref{sec:classification}, the meshes stored in the regular grid are no longer closed.
...
The idea of this approach is based on a work about finding and deforming the intersection between two meshes as well as applying textures to those regions to increase the realism in computer graphics \cite{mesh_intersection}.
...
\begin{figure}
	...
	\caption[Direct intersection concept]{
		Surface reconstruction from the data model of the VML by the example of the \cubes scene, an intersecting cube and cuboid.
...
The following sections discuss details of this algorithms and follow the order of subroutines and functions used within.
		...
		\Function{UnionStructure}{$\var{s}_1, \var{s}_2, \var{cellBox}$}
			...
			\ForAll{$(\var{t}_1, \var{t}_2) \in \var{s}_1 \times \var{s}_2$}
				...
				\If{$\var{l}$} \Comment{no intersection line may be found}
					\State $\var{lines}_{\var{t}_1}.\var{add}(\var{l})$
					\State $\var{lines}_{\var{t}_2}.\var{add}(\var{l})$
				\EndIf
			\EndFor
			\ForAll{$\var{s} \in \{\var{s}_1, \var{s}_2\}$}
				\State $\var{s'} \gets \varnothing$
				\ForAll{$\var{t} \in \var{s}$}
					\State $\var{s'} \gets \var{s'} \cup \Call{SplitTriangle}{\var{t}, \var{lines}_\var{t}}$
				\EndFor
...
\Cref{alg:clip} shows a pseudocode implementation of the \textproc{ClipStructure} routine, which is based on the Sutherland-Hodgeman algorithm.
...
The resulting triangles after clipping a structure, if any, are collected to form the new, clipped structure.
...
The input polygon itself is represented as an ordered list of vertices, where each pair of adjacent vertices forms an edge of the polygon, including the last and the first vertex as a pair.
...
In some edge cases, duplicate vertices may be generated.
...
In this case, the intersection point with the clipping plane is calculated, \cf \cref{alg:clip}, and the point is added to the result list.
		...
		\Function{Triangulate}{$\var{vertices}$}
			...
			\If{$|vertices| \geq 3$}
				...
				\For{$\var{i} \gets 2 \To |\var{vertices}| - 1$}
					\State $\var{result}.\var{add}(\var{Triangle}(\var{c}, \var{vertices}_{\var{i} - 1}, \var{vertices}_\var{i}))$
				\EndFor
			\EndIf
			\State \Return $\var{result}$
		\EndFunction
...
The intersection test of Möller \cite{tri_tri_intersection_moller}, despite being older and marginally slower than more recent developments \cite{tri_tri_intersection_2}, is available as public domain C code on his website \cite{tri_tri_intersection_moller_code}.
...
Since the code is quite long and freely available, an implementation/algorithm is not included here.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Function{IntersectTriangles}{$\var{t}_1, \var{t}_2$}
			\State $\var{hasIntersected} =$ \Call{TriTriIntersectWithIsectline}{$\hfill\break
				...
				\protect\vphantom{p}\hspace*{\dimexpr\algorithmicindent*2}\out\var{isCoplanar}, \out\var{p}_1, \out\var{p}_2$}
	...
	\caption[Möller triangle-triangle intersection adapter]{
		...
		This algorithm calls the C function \textproc{TriTriIntersectWithIsectline} with all triangle vertices as inputs and $\var{coplanar}$, $\var{p}_1$ and $\var{p}_2$ as output parameters.
	}
	\label{alg:triangle_intersection}
\end{algorithm}

The triangle intersection test of Möller itself starts with an early exit test by computing the plane equation parameters, \ie normal and distance, for both triangles.
Then, the signed plane distance of each vertex of one triangle to the plane of the other triangle is calculated.
If the signs of these three values are the same for one triangle, it completely lies on one side of the plane and therefore does not intersect the other triangle.
...
For each edge of a triangle the two signed distances of the incident vertices to the plane of the other triangle are compared.
...
For each triangle, two intersecting edges are found, thus yielding an interval for each triangle, \cf blue segments in \cref{fig:tri_intersect}.
If these two intervals overlap, the triangles intersect and their intersection line is the overlapping segment, \cf red segment in \cref{fig:tri_intersect}.
...
\begin{figure}
	...
	\caption[Möller triangle-triangle intersection test]{
		...
		The image shows intervals on the intersection line between the planes of two triangles.
		The green line is the intersection line of both planes.
		Blue parts of the intersection line are intervals of a triangle.
		The red part is the overlap of the line intervals of both triangles, \ie the intersection of both triangles.
...
This greatly simplifies several calculations, except computing the actual points of the intersection segment.
...
The result of splitting a triangle must be a new set of triangles in order to create a new structure which can be put into the \textproc{UnionStructure} routine again.
...
\begin{table}
	\centering
	\begin{tabular}{p{3cm} l l p{2.4cm} p{3.9cm}}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Function{SplitTriangle}{$\var{t}, \var{lines}$}
			...
			\EndFor
			\State $\var{cdt}(|\var{points2}|, \var{points2}, \dots)$ \Comment{Compute CDT}
...
All points used by constrained edges must be part of the input point cloud and are therefore added to the point cloud formed by the vertices of the triangle.
...
\begin{figure}[!]
	...
	\begin{subfigure}[c]{0.6\textwidth}
		\centering
		\includegraphics[width=\textwidth]{inside_test_error}
	\end{subfigure}
...
\begin{algorithm}
	...
	\caption[Triangle inside structure test]{
		...
		The \textproc{IntersectRayTriangle} function is a branch-free version of the Möller-Trumbore ray-triangle intersection test \cite{ray_triangle_intersection_moller}.
	}
...
For ray-triangle intersection the Möller-Trumbore intersection test is used \cite{ray_triangle_intersection_moller}.
...
\begin{figure}
	...
	\caption[Möller-Trumbore ray-triangle intersection test]{
		Principle of the Möller-Trumbore ray-triangle intersection test \cite{enlight_demo_workshop}.
	}
...
The necessary transformation consists of two steps.
...
If the direct intersection method would be tested on various scenes now, it would barely work for not even modest scenarios.
...
For example, when two incident triangles intersect with a third triangle, the shared edge of the first two triangles should intersect at the same point on triangle three.
...
The triangle splitting algorithm for example can only hold its guarantees if the intersecting edges are true polylines with no gaps between the segments and their ends exactly at the triangle edges.
...
\begin{description}
	...
	In situations where two triangles are quite thin, the Möller intersection test may report an intersection and calculate an intersection line which lies outside one or both of the triangles.
	...
	After all intersection lines have been recorded, collapse all constraint vertices, which are closer than a defined epsilon distance, to their mean value.
	...
	Together with the vertices of the triangle, the hull vertices are ordered cyclically around their center of mass.
...
In the underlying implementation, only the topmost loop of the \textproc{DirectIntersection} function has been parallelized using the Parallel Patterns Library (PPL) of Microsoft and the \var{parallel\_for} function, which uses a scheduler implementing work stealing to balance the workload \cite{ppl_parallel_for}.
