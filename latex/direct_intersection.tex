...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Function{Triangulate}{$\var{vertices}$}
			\State $\var{result} \gets \varnothing$
			\If{$|vertices| \geq 3$}
				\State $\var{c} \gets \var{vertices_0}$
	...
	\begin{algorithmic}[1]
		\Function{IntersectTriangles}{$\var{t_1}, \var{t_2}$}
			\State $\var{hasIntersected} =$ \Call{tri\_tri\_intersect\_with\_isectline}{$\hfill\break
				...
				\protect\vphantom{p}\hspace*{\dimexpr\algorithmicindent*2}\out\var{isCoplanar}, \out\var{p_1}, \out\var{p_2}$} \Comment{Output parameters}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Function{SplitTriangle}{$\var{t}, \var{lines}$}
			...
			\State $\var{axis_1} \gets (\var{axis_0} + 1) \bmod 3$ 
			\State $\var{axis_2} \gets (\var{axis_0} + 2) \bmod 3$
			\State $\var{points2} \gets \varnothing$ \Comment{Project 3D to 2D points, order must remain}
			\ForAll{$\var{p} \in points$}
				\State $\var{points2} \gets \var{points2} \cup \{\var{Vector2}(\var{p_{axis_1}}, \var{p_{axis_2}})\}$
			\EndFor
			...
			\For{$\var{i} \gets 0 \To \frac{|\var{indices}|}{3} - 1$}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Function{IntersectRayTriangle}{$origin, direction, t, d, u, v$}
			...
			\State $\var{d} \gets \var{edge_2} \cdot \var{qVec} \cdot \var{invDet}$
