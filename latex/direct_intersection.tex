...
flush values close to zero to zero

if a triangle intersection is recorded, test each point of the line if it is on the triangle using a raycast with the point as origin and the triangle normal as ray direction. use epsilon for u and v.

after all cutting segments have been recorded for each triangle: make robust chains.
collapse cutting edges' vertices with structure vertices
collapse near points in cutting edges
ensure unique cutting edges

hull constrains to avoid degenerates at the convex hull
still check for degenerate triangles and remove them, if dot of angle at any triangle corner is below epsilon.
