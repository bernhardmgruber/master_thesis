...
Three examples of algorithms for surface reconstruction from point clouds are given in the following:
...
\begin{figure}
	...
	\end{subfigure}
	\caption{
		Details of the cylinder\_head and turbine scene after BPA reconstruction rendered using MeshLab.
		Boundary edges are marked in green.
		The left rendering shows the creation of holes and non-manifold triangles between two fins of the cylinder\_head, which may happen if the ball is roughly as big as a concave feature.
		The right rendering shows a cog of the turbine where the ball has fell inside the point cloud.
		Between the differently oriented meshes, lots of boundary edges and overlapping triangles are created.
	}
	\label{fig:bpa_issues}
\end{figure}

table with holes created by BPA

Concerning the created boundary edges and holes, \cf table \ref{tbl:bpa_boundary edges}, the BPA is quite robust if the point cloud does not contain concave features which are slightly larger then the size of the ball.
%
\begin{table}
	\centering
...
In all of these cases, the ball rolls inside a concave feature/hole and creates triangles spanning the sides of the feature, \cf figure \ref{fig:bpa_cylinder_head_non_manif}.
