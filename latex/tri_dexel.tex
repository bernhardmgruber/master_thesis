...
Algorithm \ref{alg:tri_dexel_refinement} describes the details of the refinement and feature reconstruction pass, creating enhanced boundary loops which are also triangulated.
	...
	\begin{algorithmic}[1]
		\Function{TriangulateCellRefined}{$\var{loop}, \var{cell}$}
			...
			\For{$i \gets 0 \To |loop| - 1$}
				...
				\State $points \gets \Call{Unique}{\Call{Sort}{(a.point, a.neigbor, b.point, b.neighbor)}}$
				\State $idx \gets planeIndex_{points_0, points_1, points_2}$
				\State $n \gets cellPlaneNormals_{idx}$
				...
				\If{$intermed \wedge box.lower < intermediate < box.upper$}
					\State $dist \gets \Call{PointLineDistance}{Line(a.position, b.position), intermed}$
					\If{$dist > \delta$}
			...
			\If{$intermedCount \geq 3$}
				...
				\If{$a \wedge box.lower < a < box.upper$}
					\State $apex \gets a$
				\EndIf
			\EndIf
			\State $\dots$
