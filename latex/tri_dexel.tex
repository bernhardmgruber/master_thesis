...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Function{CreateRay}{$\var{box}, \var{axis0}, \var{axis1}, \var{axis2}, \var{\Delta x}, \var{\Delta y}, \var{x}, \var{y}, \var{xCount}, \var{yCount}$}
			...
			\State $\var{origin}_{\var{axis1}} \gets \var{origin}_{\var{axis1}} + \var{x} * \var{\Delta x}$
			\State $\var{origin}_{\var{axis2}} \gets \var{origin}_{\var{axis2}} + \var{y} * \var{\Delta y}$
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Function{FindDividers}{$\var{cell}$}
			\State $\var{rangesPerAxis} \gets \var{array}(3, \var{array}())$
			\For{$ \var{i} \gets 0 \To 11 $}
				...
				\If{$(\var{srcOcc} \wedge \var{dstOcc} \wedge |\var{segs}| > 1) \vee (\neg \var{srcOcc} \wedge \neg \var{dstOcc} \wedge |\var{segs}| > 0) \vee (\var{srcOcc} \neq \var{dstOcc} \wedge |\var{segs}| > 1)$}
					\State $\var{rangesPerAxis}_{\var{axis}}.\var{add}(\Call{SegmentsToRanges}{\var{segs}, \var{srcDepth}, \var{dstDepth}})$
				\EndIf
			\EndFor
			\State $\var{dividers} \gets (\var{array}(), \var{array}(), \var{array}())$
			\For{$\var{axis} \gets 0 \To 2$}
				\State $\var{ranges} \gets \var{rangesPerAxis}_{\var{axis}}$
				\State $\Call{Sort}{\var{ranges}}$ \Comment{Sort by range start}
		...
		\Function{SegmentsToRanges}{$\var{segs}, \var{srcDepth}, \var{dstDepth}$}
			...
			\ForAll{$\var{seg} \in \var{segs}$}
				\If{$\var{seg}.\var{start}.\var{depth} \neq \var{srcDepth}$}
					\State $\var{depths}.\var{add}(\var{seg}.\var{start}.\var{depth})$
				\EndIf
				\If{$\var{seg}.\var{end}.\var{depth} \neq \var{dstDepth}$}
					\State $\var{depths}.\var{add}(\var{seg}.\var{end}.\var{depth})$
				\EndIf
			\EndFor
