...
Converting the tri-dexel image into a tri-dexel grid is done by the \textproc{CreateTriDexelGrid} function, which is shown in \cref{alg:tri_dexel_grid_generation}.
%
\begin{algorithm}
	\centering
	\begin{algorithmic}[1]
...
The triangulation shown in the last \cref{sec:tri_dexel_triangulation}, \textproc{TriangulateLoopSimple} in \cref{alg:tri_dexel_triangulation}, is very basic and does not make use of the per-vertex normal information.
...
In the left drawing of \cref{fig:tri_dexel_refinement}, the original boundary loop consists of the vertices A, B, C, and D.
...
The reconstruction proceeds analogously to the one of the main tri-dexel image, \cf \cref{alg:tri_dexel} \cref{line:tri_dexel_begin}ff. %TODO
...
The depth ranges are computed for each axis, by iterating over all 12 edges of the cell and computing the same variables as in the regularization \cref{alg:tri_dexel_regularization}, \crefrange{line:reg_vars_begin}{line:reg_vars_end}.
...
Finally, the tri-dexel surface reconstruction approach described in this chapter has been tested on the scenes described in \cref{sec:test_scenes}.
