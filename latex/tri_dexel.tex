...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Function{DepthFirstSearch}{$occupied, cur, last, cell, visited, loop$}
			\If{$cell.point_{cur} = occupied \wedge \neg visited_{cur}$}
				\State $visited_{cur} \gets \True$
				\State $neighbors \gets \Call{Rotate}{neighborIds_{cur}, last}$
				\For{$n \in neighbors$}
					\If{$cell.point_{n} \neq occupied$}
						...
						\If{$seg.start.depth = curReal_{axis} \vee seg.start.depth = nReal_{axis}$}
							\State $v_{axis} \gets seg.end.depth$
							\State $loop.add(EdgeVertex(cur, n, v, seg.end.normal))$
						\Else
							\State $v_{axis} \gets seg.start.depth$
							\State $loop.add(EdgeVertex(cur, n, v, seg.start.normal))$
						\EndIf
					\Else
