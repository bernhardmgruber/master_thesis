...
\Cref{fig:cylinder_head_dexel} shows the \cylinderhead scene with a low-resolution dexel image and the final reconstruction.
...
\begin{figure}
	...
	\caption[Tri-dexel concept]{
		Tri-dexel based surface reconstruction from the data model of the VML of the \cylinderhead scene.
		...
		Renderings of details of the tri-dexel image in \subref{fig:cylinder_head_dexel_image_center} and~\subref{fig:cylinder_head_dexel_image_fins} show the drilling at the center from above and the fins of the \cylinderhead from the center.
		Finally, the reconstructed surface is shown in \subref{fig:cylinder_head_reconstructed}, containing several imperfections at the edges and bases of the fins.
	}
...
A cell stores its bounding box in \var{aabb}, the point occupancy of each of the corners corners in \var{occupancy}, the coordinates of the corners in \var{realPoints} and the twelve edges of the cell in \var{edges}.
...
A \var{DexelImage} describes the result of a single raycast along the axis specified as integer in \var{axis0}, where 0 denotes the x-, 1 the y- and 2 the z-axis.
...
These contain two nodes, a \var{start} and \var{end} node.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Function{TriDexel}{$\var{grid}, \var{resolution}$}
			\State $\var{box} = \var{BoundingBox}(\var{grid}.\var{aabb}.\var{lower} - \epsilon_{\var{box}}, \var{grid}.\var{aabb}.\var{upper} + \epsilon_{\var{box}})$
			\State $\var{res} = \Call{UniformResolution}{\var{box}, \var{resolution}}$
			\State $\Call{Reconstruct}{\var{grid}, \var{box}, \var{res}}$
		\EndFunction
		\\
		\Function{Reconstruct}{$\var{grid}, \var{box}, \var{res}$}
...
The longest one is equal to the specified resolution, the other two are calculated in such a way that the resulting cells of the tri-dexel grid are as cubic as possible.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\EndProcedure
		\\
		\Function{CreateRay}{$\var{box}, \var{axis0}, \var{axis1}, \var{axis2}, \var{\Delta x}, \var{\Delta y}, \var{x}, \var{y}$}
...
\var{axis0} is also called primary axis and is accompanied by \var{axis1} and \var{axis2} which hold the two secondary axes, in cyclic order.
...
Thus, \var{x} and \var{y} are multiplied by the distances between incident rays, \var{\Delta x} and \var{\Delta y}, and added to the components at the secondary axes of the origin.
%
...
This method is documented well in the tri-dexel work foundational to this implementation \cite{tridexel_reconstruction}.
	...
	\begin{algorithmic}[1]
		\Function{RegularizeCell}{$\var{cell}$}
			\For{$ \var{i} \gets 0 \To 11 $}
				\State $\var{segs} \gets \var{cell}.\var{edges}_{\var{i}}.\var{segments}$ \Comment{reference only}
...
Firstly, a set of boundary loops is created from the grid points and dexel nodes between them.
...
\Cref{fig:tri_dexel_triangulation} illustrates this depth-first search process by two examples.
%
\begin{figure}
	\centering
...
The \textproc{FindLoops} algorithm is the entry into the depth-first search based traversal of the edges of the cell, \cf \cref{fig:tri_dexel_triangulation}.
...
After the call has returned, a conditional checks if loop vertices have been found and adds the loop in such a case.
...
If this is the case, then \var{cur} and \var{n} have different occupancy and the edge between them contains a vertex of the surface.
...
The \textproc{TriangulateLoopIntoFan} function is one of three loop triangulation primitives used throughout this implementation.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\EndFunction
		\\
		\Function{TriangulateLoopAtFirst}{$\var{loop}$}
			\If{$|\var{loop}| < 3$}
				\State \Return $\varnothing$
			\EndIf
...
The function initially checks if the loop contains at least three triangles and returns an empty set of triangles in such a case.
...
Finally, after creating intermediate and apex vertices, the enhanced boundary loops are triangulated.
...
\begin{algorithm}
	...
	\caption[Feature reconstruction 1]{
		Feature reconstruction by calculating intermediate vertices and an optional apex vertex based on the original boundary loops.
...
The case illustrated in \cref{fig:tri_dexel_refinement} on the left shows that the final loop must actually be split into two sub loops, one containing C, D, H and F, as well as one containing A, B, D, H.
...
Therefore, in this case, the function does not return a result.
...
This value is the doubled area of the triangle \var{a}, \var{b}, \var{p}, where the line distance is the altitude of the triangle.
...
The reason therefore is the destructive regularization step described in \cref{sec:tri_dexel_regularization}, which sacrifices smaller dexel segments in favor of regularity, throwing away useful information, especially of smaller features.
...
\begin{figure}
	...
	\caption[Cell slicing creating a hole]{
		...
		The light red area marks the hole created at the border of both cells.
	}
...
Thus, a different surface is created at the border, resulting in a hole.
...
The reconstruction procedure is similar to the algorithm forming the main tri-dexel image, \cf \cref{alg:tri_dexel} \cref{line:tri_dexel_begin}ff.
...
A Boolean return value of \textproc{RegularizeCell} informs the caller if irregular edges have been detected.
	...
	\begin{algorithmic}[1]
		\Function{SliceCell}{$\var{cell}, \var{grid}$}
			...
			\ForAll{$\var{x} \gets 0 \To |\var{xdiv}| - 2, \var{y} \gets 0 \To |\var{ydiv}| - 2, \var{z} \gets 0 \To |\var{zdiv}| - 2$}
				...
				\State $\var{box} \gets \var{BoundingBox}(\var{lower}, \var{upper})$
...
Concerning the raycast implementation, all three for loops in \cref{alg:tri_dexel_raycast} are viable candidates for parallel execution.
...
The remaining algorithms still offer a small degree of parallelism.
...
In the underlying implementation, parallelization has been applied where suitable, again using the PPL of Microsoft and the \var{parallel\_for} primitive \cite{ppl_parallel_for}.
