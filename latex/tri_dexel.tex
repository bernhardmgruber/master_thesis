...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Function{TriDexel}{$\var{grid}, \var{resolution}$}
			...
			\State \Call{Reconstruct}{$grid, box, resolution$}
		\EndFunction
		\\
		\Function{Reconstruct}{$grid, box, resolution$}
...
\subsection{Cell slicing *experimential*}
...
\begin{figure}
	\centering
	\includegraphics[width=0.9\textwidth]{images/tri_dexel_cellslicing}
	\caption{
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Function{Reconstruct}{$grid, box, resolution$}\hfill\break
			...
			\setalglineno{11}
			\ForAll{$\var{c} \in \var{dgrid}.\var{cells}()$}
				\If{$\Call{RegularizeCell}{\var{c}, \True}$}
					\State $\var{triangles} \gets \var{triangles} \cup \Call{TriangulateCell}{\var{c}}$
				\Else
...
\begin{algorithm}
	\centering
	\begin{algorithmic}[1]
		\Function{SliceCell}{$cell, grid$}
			\State $(xdiv, ydiv, zdiv) \gets \Call{FindDividers}{cell}$
			\State $triangles \gets \varnothing$
			\ForAll{$x \gets 0 \To |xdiv| - 2, y \gets 0 \To |ydiv| - 2, z \gets 0 \To |zdiv| - 2$}
		...
		\EndFunction
		\\
		\Function{SegmentsToRanges}{$segs, srcDepth, dstDepth$}
...
\begin{figure}
	...
	\end{subfigure}
	\caption{
		Renderings of the result meshes using MeshLab after applying the tri-dexel reconstruction approach on the selected test scenes in table \ref{tbl:test_scenes}.
	}
