	...
	\begin{algorithmic}[1]
		\algrestore{refinement}
			...
			\ElsIf{$intermedCount = 1$}
				...
				\State \Return $\Call{TriangulateLoopAtFirst}{finalLoop}$
			\Else
				\State $triangles \gets \varnothing$
				\For{$first \gets 0 \To |finalLoop| - 1$} \Comment{Reevaluated at each iteration}
					\If{$\neg isEdgeVertex_{first}$}
						...
						\State $\Call{RemoveCyclicRange}{finalLoop, first + 1, last - 1}$
						\State $\Call{RemoveCyclicRange}{isEdgeVertex, first + 1, last - 1}$
					\EndIf
				\EndFor
...
In this case, the function does not return a result, \ie a result which always evaluates to \False in boolean expressions.
