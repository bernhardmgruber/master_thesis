...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Function{TriangulateLoopSimple}{$loop$}
			...
			\State \Return $\Call{TriangulateLoopIntoFan}{vertices}$
		\EndFunction
	\end{algorithmic}
	\caption{
...
\begin{algorithm}
	\centering
	\begin{algorithmic}[1]
		\Function{TriangulateLoopIntoFan}{$loop, center$}
			\If{$|loop| < 3$}
				\State \Return $\varnothing$
			\EndIf
			\State $triangles \gets \varnothing$
			\For{$a \gets 0 \To |loop| - 1$}
				\State $b \gets (a + 1) \mod |loop|$
				\State $triangles.add(Triangle(center, loop_{a}, loop_{b}))$
			\EndFor
			\State \Return $triangles$
		\EndFunction
		\\
		\Function{TriangulateLoopIntoFan}{$loop$}
			\State $center \gets \Call{Sum}{loop} \div |loop|$
			\State \Return $\Call{TriangulateLoopIntoFan}{loop, center}$
		\EndFunction
		\\
		\Function{TriangulateLoopAtFirst}{$loop, center$}
			\If{$|loop| < 3$}
				\State \Return $\varnothing$
			\EndIf
			\State $center \gets loop_0$
			\State $triangles \gets \varnothing$
			\For{$b \gets 2 \To |loop| - 1$}
				\State $a \gets (b - 1)$
				\State $triangles.add(Triangle(center, loop_{a}, loop_{b}))$
			\EndFor
			\State \Return $triangles$
...
\begin{algorithm}
	\centering
	\begin{algorithmic}[1]
		\Function{TriangulateCellRefined}{$\var{loop}, \var{cell}$}
			\State $inside \gets p \rightarrow cell.aabb.lower < p < cell.aab.upper$
			\State $apexVertices \gets array()$
			\State $finalLoop \gets array()$
			\State $isEdgeVertex \gets array()$
			\State $intermedCount = 0$
			\LineComment{Create new boundary loop with intermediate vertices}
			\For{$i \gets 0 \To |loop| - 1$}
				\State $a \gets loop_{i}$
				\State $b \gets loop_{(i + 1) \mod |loop|}$
				\State $finalLoop.add(a.position)$
				\State $isEdgeVertex.add(\True)$
				\State $points \gets \Call{Unique}{(a.point, a.neigbor, b.point, b.neighbor)}$
				%\State $idx \gets \Call{PlaneIndex}{points_0, points_1, points_2}$
				\State $n \gets cellPlaneNormals_{points_0, points_1, points_2}$
				\State $cellPlane \gets Plane(n, cell.realPoints_{points_0})$
				\State $aPlane \gets Plane(a.normal, a.position)$
				\State $bPlane \gets Plane(b.normal, b.position)$
				\State $intermed \gets \Call{IntersectPlanes}{cellPlane, aPlane, bPlane}$
				\If{$intermed \wedge inside(intermediate)$}
					\State $dist \gets \Call{PointLineDistance}{a.position, b.position, intermed}$
					\If{$dist > \delta$}
						\State $finalLoop.add(intermed)$
						\State $isEdgeVertex.add(\False)$
						\State $intermedCount \gets intermedCount + 1$
						\State $apexVertices.add(a)$
						\State $apexVertices.add(b)$
					\EndIf
				\EndIf
			\EndFor
			\LineComment{Try to create an apex vertex}
			\If{$intermedCount \geq 3$}
				\State $apexVertices \gets \Call{Unique}{apexVertices}$
				\State $planes \gets array()$
				\For{$i \gets 0 \To 2$}
					\State $planes_i \gets Plane(apexVertices_i.normal, apexVertices_i.position)$
				\EndFor
				\State $a \gets \Call{IntersectPlanes}{planes_0, planes_1, planes_2}$
				\If{$a \wedge inside(a)$}
					\State $apex \gets a$
				\EndIf
			\EndIf
...
One contains a list of vertices which have lead to a valid intermediate point and may be considered for creating an apex vertex, called \var{apexVertices}.
...
This second part of the \textproc{TriangulateCellRefined} function is listed in algorithm \ref{alg:tri_dexel_refinement_triangulation}.
%
\begin{algorithm}
	\centering
	\begin{algorithmic}[1]
		\algrestore{refinement}
			\State $\dots$
			\If{$apex$}
				\State \Return $\Call{TriangulateLoopIntoFan}{finalLoop, apex}$
			\ElsIf{$intermedCount = 0$}
				\State \Return $\Call{TriangulateLoopIntoFan}{finalLoop}$
			\ElsIf{$intermedCount = 1$}
				\State $\Call{Rotate}{finalLoop, \Call{FindFirst}{isEdgeVertex, \False}}$
				\State \Return $\Call{TriangulateLoopIntoFan}{finalLoop}$
			\Else
				\State $triangles \gets \varnothing$
				\For{$first \gets 0 \To |finalLoop| - 1$} \Comment{Reevaluated at each iteration}
					\If{$\neg isEdgeVertex_{first}$}
						\State $last \gets (first + 1) \mod |finalLoop|$
						\While{$isEdgeVertex_{last}$}
							\State $last \gets (last + 1) \mod |finalLoop|$
						\EndWhile
						\State $subLoop \gets array()$
						\For{$i \gets (first \To last) \mod |finalLoop|$}
							\State $subLoop.add(finalLoop_{i})$
						\EndFor
						\State $subLoop.add(finalLoop_{last})$
						\State $triangles \gets triangles \cup \Call{TriangulateLoopIntoFan}{subLoop}$
						\State $\Call{RemoveCyclicRange}{finalLoop, first + 1, last}$
						\State $\Call{RemoveCyclicRange}{isEdgeVertex, first + 1, last}$
					\EndIf
				\EndFor
				\State $triangles \gets triangles \cup \Call{TriangulateLoopIntoFan}{finalLoop}$
				\State \Return $triangles$
			\EndIf
		\EndFunction
		\\
		\Function{IntersectPlanes}{$a, b, c$}
			\State $det \gets \begin{vmatrix} a.normal & b.normal & c.normal \end{vmatrix}$
			\If{$det \neq 0$}
				\State \Return $((b.normal \times c.normal) \cdot a.dist + {}$\hfill\break
				\hspace*{\dimexpr\algorithmicindent*2}\phantom{\Return $($}$(c.normal \times a.normal) \cdot b.dist + {}$\hfill\break
				\hspace*{\dimexpr\algorithmicindent*2}\phantom{\Return $($}$(a.normal \times b.normal) \cdot c.dist) \div det$
			\EndIf
			\LineComment{Otherwise, return nothing}
		\EndFunction
		\\
		\Function{PointLineDistance}{$a, b, p$}
			\State \Return $|(p - a) \times (p - b)| \div |a - b|$
		\EndFunction
	\end{algorithmic}
	\caption{
